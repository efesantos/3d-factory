...
| Structure | at least one metric anchor per room harvested from public evidence (see the anchor rule in section 4), or a recorded range with a certainty label; one chirality sentence per room; no dimension contradicts the plan or the listing | code + ledger review |
...
| Fidelity critic | Gemini 3.8 Flash via OpenRouter (the only Gemini route; there is no native Gemini key) | Gemini 3.1 Pro via OpenRouter | Qwen3-VL via OpenRouter, advisory only (it missed most sabotages) | pause review; never fall back to the author's vendor |
...
- **Anchor rule.** Nothing is measured on site and Elmer has no sources beyond what is public, so the metric anchor is a task, not a request to Elmer. The anchor-hunt task collects every public constraint with a stated tolerance: room or total floor area in the listing text; a stated bed or mattress size; standard door-leaf widths for the country and building type; switch plates, sockets, tiles, radiators, appliances and branded products with published dimensions; the plan's own scale bar or dimension strings. Several weak anchors beat one assumed one (bedroom-A's assumed mattress size was the error). The joint camera fit then reconciles them. If they still disagree, the dimension stays `inferred` with its range and certainty label, and the approximation policy applies: nothing built may contradict any of the constraints. One chirality sentence per room before the shell is built. Clean the crop before any generation spend.
...
1. **Bedroom A passes the blind bar.** Lighting re-tune against real albedos (≈20× light budget, exposure matched to photo luminance ≈0.21), curtains as cloth, a window view instead of white sky, headboard and lamps from clean crops, and the anchor hunt for the width under the anchor rule. Re-run the perceptual packets, then the four sabotages on Cycles renders to calibrate the critics. Needs from Elmer: the next blind judgement.
